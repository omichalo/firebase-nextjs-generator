...
export const useFCM = () => {
  ...
  const requestToken = useCallback(async () => {
    if (!isSupported || !FCM_CONFIG.vapidKey) {
      setError('FCM non supporté ou clé VAPID manquante');
      return;
    }
    ...
    (notification: FCMNotification) => {
      ...
      try {
        const notificationOptions: NotificationOptions = {
          ...
          badge:
            notification.badge || FCM_CONFIG.defaultNotificationOptions.badge,
          tag: notification.tag,
          data: notification.data,
          ...(notification.actions && { actions: notification.actions }),
          // Note: vibrate n'est pas supporté dans notre configuration actuelle
          ...(FCM_CONFIG.defaultNotificationOptions.requireInteraction !==
            undefined && {
            requireInteraction:
              FCM_CONFIG.defaultNotificationOptions.requireInteraction,
          }),
          ...(FCM_CONFIG.defaultNotificationOptions.silent !== undefined && {
