...
export class NextJSGenerator {
  ...
  async generate(): Promise<void> {
    ...
    await this.generateTypeScriptConfig(outputDir);

    // Générer la configuration Next.js
    await this.generateNextJSConfig(outputDir);

    // Générer les composants
    await this.generateComponents(outputDir);

    // Générer les stores (Zustand ou Redux)
    await this.generateStores(outputDir);

    // Générer les hooks personnalisés
    await this.generateCustomHooks(outputDir);

    // Générer les routes API
