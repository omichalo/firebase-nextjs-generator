...
export class NextJSGenerator {
  ...
  private async generateDevScript(outputDir: string): Promise<void> {
    ...
    let devScriptContent = await fs.readFile(devScriptPath, 'utf-8');

    // Remplacer les dépendances selon la gestion d'état
    if (this.options.nextjs.stateManagement === 'redux') {
      // Remplacer zustand par redux
      devScriptContent = devScriptContent.replace(
        'npm install zustand',
    ...
    if (this.options.nextjs.features.sentry) {
      devScriptContent = devScriptContent.replace(
        'npm install clsx',
        'npm install clsx @sentry/nextjs'
      );
    }

    // Écrire le script de démarrage modifié
    await fs.writeFile(path.join(outputDir, 'dev.sh'), devScriptContent);
  }
