...
export class FirebaseGenerator {
  ...
  private async generateMigrations(firestoreDir: string): Promise<void> {
    ...
    // Créer au moins une migration par défaut si aucune n'est configurée
    if (
      !this.options.firestore.migrations ||
      this.options.firestore.migrations.length === 0
    ) {
      const defaultMigration = {
        version: 1,
        description: 'initial-schema',
        up: '// Migration initiale',
        down: '// Rollback initial',
      };
      ...
      await this.templateEngine.processTemplate(
        ...
        path.join(
          migrationsDir,
          `v${defaultMigration.version}-${defaultMigration.description}.ts`
        ),
        { migration: defaultMigration }
      );
    } else {
      // Générer les migrations configurées
  ...
  private async generateFirebaseExtensions(outputDir: string): Promise<void> {
    ...
    // Créer au moins une extension par défaut si aucune n'est configurée
    if (
      !this.options.firebase.extensions ||
      this.options.firebase.extensions.length === 0
    ) {
      ...
      const extensionDir = path.join(extensionsDir, defaultExtension.name);
      ...
      // Générer les extensions configurées
  ...
  private async generateDeploymentScripts(outputDir: string): Promise<void> {
    console.log('🔧 Génération des scripts de déploiement...');
    const scriptsDir = path.join(outputDir, 'scripts');
    console.log(`📁 Répertoire des scripts: ${scriptsDir}`);

    try {
      // Script de déploiement principal
      console.log('📝 Génération du script deploy.sh...');
      await this.templateEngine.processTemplate(
        path.join(this.options.templateDir, 'firebase/scripts/deploy.sh.hbs'),
        path.join(scriptsDir, 'deploy.sh')
      );
      console.log('✅ Script deploy.sh généré');

      // Script de déploiement par environnement
      for (const env of this.options.firebase.environments) {
        console.log(`📝 Génération du script deploy-${env.name}.sh...`);
        await this.templateEngine.processTemplate(
          path.join(
            this.options.templateDir,
        ...
        console.log(`✅ Script deploy-${env.name}.sh généré`);
      }

      // Script de rollback (optionnel)
      try {
        console.log('📝 Génération du script rollback.sh...');
        await this.templateEngine.processTemplate(
          path.join(
            this.options.templateDir,
            'firebase/scripts/rollback.sh.hbs'
          ),
          path.join(scriptsDir, 'rollback.sh')
        );
        console.log('✅ Script rollback.sh généré');
      } catch (error) {
        // Si le template rollback n'existe pas, on l'ignore
        console.log('⚠️ Template rollback.sh non trouvé, ignoré');
      }

      // Rendre les scripts exécutables
      console.log('🔒 Rendre les scripts exécutables...');
      const scripts = await fs.readdir(scriptsDir);
      for (const script of scripts) {
        if (script.endsWith('.sh')) {
          await fs.chmod(path.join(scriptsDir, script), 0o755);
          console.log(`✅ Script ${script} rendu exécutable`);
        }
      }

      console.log(
        '🎉 Tous les scripts de déploiement ont été générés avec succès!'
      );
    } catch (error) {
      console.error(
        '❌ Erreur lors de la génération des scripts de déploiement:',
        error
      );
      throw error;
    }
