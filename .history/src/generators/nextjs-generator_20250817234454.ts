...
export class NextJSGenerator {
  ...
  private async generatePackageJson(outputDir: string): Promise<void> {
    // Lire le template package.json et le traiter avec Handlebars
    ...
    // Traiter le contenu avec Handlebars pour remplacer les variables
    const processedContent = await this.templateEngine.processContent(
      packageJsonContent,
      {
        project: {
          name: this.options.project.name,
          description: this.options.project.description,
          author: this.options.project.author,
          license: this.options.project.license,
          packageManager: this.options.project.packageManager,
        },
        nextjs: {
          version: this.options.nextjs.version,
        },
      }
    );

    // Écrire le package.json traité
    await fs.writeFile(path.join(outputDir, 'package.json'), processedContent);
  }
