...
async function checkFirebaseProject(
  ...
  try {
    ...
    try {
      execSync(
        "firebase projects:list --token $(firebase login:ci --no-localhost)",
        { stdio: "pipe" }
      );
    } catch {
      return {
        exists: false,
        error:
          "Utilisateur Firebase non connecté. Connectez-vous avec: firebase login",
      };
    }
      ...
      if (output.includes(projectId)) {
        ...
        try {
          const projectInfo = execSync(
            `firebase projects:list --token $(firebase login:ci --no-localhost) --format json`,
            {
              stdio: "pipe",
              encoding: "utf8",
            }
...
async function promptForEnvironments(): Promise<any[]> {
  ...
  for (const envName of environments) {
    ...
    if (projectType === "link") {
      ...
      // Pour un projet existant, on ne demande PAS la région (elle est déjà définie)
      region = "existing"; // Placeholder, sera ignoré lors de la génération

      console.log(chalk.green(`✅ Projet lié : ${projectId}`));
    } else {
