...
`;
  ...
  private processTemplate(template: string): string {
    // Simple template processing - in a real implementation, use Handlebars
    return template
      .replace(/\{\{project\.name\}\}/g, this.options.project.name)
      .replace(
        /\{\{project\.description\}\}/g,
        this.options.project.description
      )
      .replace(
        /\{\{project\.packageManager\}\}/g,
        this.options.project.packageManager
      )
      .replace(
        /\{\{firebase\.projectId\}\}/g,
        this.options.firebase.projectId || "default-project"
      )
      .replace(/\{\{nextjs\.version\}\}/g, this.options.nextjs.version)
      .replace(/\{\{nextjs\.ui\}\}/g, this.options.nextjs.ui)
      .replace(
        /\{\{nextjs\.stateManagement\}\}/g,
        this.options.nextjs.stateManagement
      )
      .replace(
        /\{\{#ifEquals nextjs\.ui "mui"\}\}(.*?)\{\{\/ifEquals\}\}/g,
        this.options.nextjs.ui === "mui" ? "$1" : ""
      )
      .replace(
        /\{\{#ifEquals nextjs\.ui "shadcn"\}\}(.*?)\{\{\/ifEquals\}\}/g,
        this.options.nextjs.ui === "shadcn" ? "$1" : ""
      )
      .replace(
        /\{\{#ifEquals nextjs\.stateManagement "zustand"\}\}(.*?)\{\{\/ifEquals\}\}/g,
        this.options.nextjs.stateManagement === "zustand" ? "$1" : ""
      )
      .replace(
        /\{\{#if nextjs\.features\.pwa\}\}(.*?)\{\{\/if\}\}/g,
        this.options.nextjs.features.pwa ? "$1" : ""
      )
      .replace(
        /\{\{#if nextjs\.features\.fcm\}\}(.*?)\{\{\/if\}\}/g,
        this.options.nextjs.features.fcm ? "$1" : ""
      );
  }
}
