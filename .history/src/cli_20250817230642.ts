...
program
  ...
  .option('--author <author>', 'Auteur du projet')
  .option(
    '--package-manager <manager>',
    'Gestionnaire de paquets (npm|yarn|pnpm)'
  )
  .option('--nextjs-version <version>', 'Version Next.js (14|15)')
  .option('--ui <framework>', 'Framework UI (mui|shadcn|both)')
  .option('--state-management <manager>', "Gestion d'état (zustand|redux|both)")
  .option(
    '--features <features>',
...
async function promptForConfiguration(
  ...
  // Mode non-interactif si --yes est spécifié
  if (cliOptions.yes) {
    console.log(
      chalk.blue('🚀 Générateur Firebase + Next.js avec App Hosting')
    );
    console.log(chalk.gray('Mode non-interactif activé...\n'));

    // Utiliser directement les options CLI
    const answers = {
      projectName: cliOptions.name || 'test-project',
      description: cliOptions.description || 'Test project',
      author: cliOptions.author || 'Test User',
      packageManager: cliOptions.packageManager || 'npm',
      nextjsVersion: cliOptions.nextjsVersion || '15',
      uiFramework: cliOptions.ui || 'mui',
      stateManagement: cliOptions.stateManagement || 'zustand',
      features: cliOptions.features ? cliOptions.features.split(',') : ['pwa'],
      outputDir: cliOptions.output || './test-output',
    };

    // Validation des options CLI
    if (answers.projectName && !/^[a-z0-9-]+$/.test(answers.projectName)) {
      throw new Error(
        `Nom de projet invalide: "${answers.projectName}". Le nom doit contenir uniquement des lettres minuscules, chiffres et tirets.`
      );
    }

    if (answers.description && answers.description.length < 10) {
      throw new Error(
        `Description trop courte: "${answers.description}". La description doit faire au moins 10 caractères.`
      );
    }

    // Configuration des environnements (mode non-interactif)
    const environments = await promptForEnvironmentsNonInteractive();

    // Configuration des extensions Firebase (mode non-interactif)
    const extensions = await promptForExtensionsNonInteractive();

    // Configuration des thèmes (mode non-interactif)
    const themes = await promptForThemesNonInteractive();

    return buildConfiguration(answers, environments, extensions, themes);
  }
...
// Fonctions non-interactives pour le mode --yes
async function promptForEnvironmentsNonInteractive(): Promise<any[]> {
...
async function promptForExtensionsNonInteractive(): Promise<any[]> {
  // Pas d'extensions par défaut pour les tests
  return [];
}

async function promptForThemesNonInteractive(): Promise<any[]> {
  // Thème par défaut pour les tests
  return [
    {
      name: 'default',
      type: 'mui',
...
async function generateProject(config: GeneratorOptions): Promise<void> {
  ...
  try {
    ...
    // Afficher les environnements Firebase configurés
    console.log(chalk.blue(`🔥 Environnements Firebase:`));
    config.firebase.environments.forEach(env => {
      console.log(
        chalk.blue(`   📍 ${env.name}: ${env.projectId} (${env.region})`)
      );
    });
    ...
    console.log(chalk.gray('1. cd ' + config.outputDir + '/frontend'));
    console.log(chalk.gray('2. npm install'));
    console.log(chalk.gray('3. cd ../backend'));
    console.log(chalk.gray('4. firebase login'));
    console.log(
      chalk.gray('5. firebase use [environment] (ex: firebase use dev)')
    );
    console.log(chalk.gray('6. cd ../frontend'));
    console.log(chalk.gray('7. npm run dev'));

    console.log(
      chalk.blue('\n💡 Note: Le projet est structuré en frontend/ et backend/')
    );
    console.log(chalk.blue('   - Frontend: Application Next.js'));
    console.log(chalk.blue('   - Backend: Configuration Firebase'));

    console.log(chalk.green('\n🚀 Initialisation automatique (recommandé):'));
    console.log(chalk.gray('   cd ' + config.outputDir));
    console.log(chalk.gray('   chmod +x scripts/init-project.sh  # Linux/Mac'));
    console.log(chalk.gray('   ./scripts/init-project.sh [environment]'));
    console.log(chalk.gray('   # Ou sur Windows:'));
    console.log(chalk.gray('   scripts\\init-project.bat [environment]'));
  } catch (error) {
