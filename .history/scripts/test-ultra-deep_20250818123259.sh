...
# Arrêter le serveur
# Arrêter le processus de développement de manière plus robuste
if [ -n "$DEV_PID" ] && kill -0 "$DEV_PID" 2>/dev/null; then
    debug_log "Stopping development server (PID: $DEV_PID)..."
    kill "$DEV_PID" 2>/dev/null || true
    # Attendre un maximum de 5 secondes
    for i in {1..5}; do
        if ! kill -0 "$DEV_PID" 2>/dev/null; then
            debug_log "Development server stopped successfully"
            break
        fi
        sleep 1
    done
    ...
    if kill -0 "$DEV_PID" 2>/dev/null; then
        debug_log "Force killing development server..."
        kill -9 "$DEV_PID" 2>/dev/null || true
    fi
else
    debug_log "Development server already stopped or PID invalid"
fi
...
echo -e "${YELLOW}🔄 Progression: 12/12 phases (100%)${NC}"
echo -e "${BLUE}⏱️  Finalisation en cours...${NC}"
