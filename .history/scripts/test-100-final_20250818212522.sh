...
npm run dev:dynamic > dev.log 2>&1 &
DEV_PID=$!
...
if curl -s http://localhost:3000 | grep -q "Test Page\|Hello World"; then
...
if curl -s http://localhost:3000/auth/login | grep -q "Connexion\|Login"; then
...
if curl -s http://localhost:3000/dashboard | grep -q "Dashboard\|Tableau"; then
