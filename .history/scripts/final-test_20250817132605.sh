...
# Script de test final pour le Générateur Firebase + Next.js 2025
# Usage: ./scripts/final-test.sh
...
echo -e "${BLUE}=== Test Final du Générateur Firebase + Next.js 2025 ===${NC}"
echo
...
if npx ts-node src/cli.ts create --name test-complete --description "Complete test project" --author "Test" --version "1.0.0" --package-manager npm --nextjs-version 15 --ui shadcn --state-management redux --features pwa,fcm,analytics,performance,sentry --output ./test-output-complete --yes; then
    ...
    TOTAL=$((TOTAL + 1))
    if [ -f "test-output-complete/frontend/public/manifest.json" ] && [ -f "test-output-complete/frontend/src/fcm/fcm-config.ts" ]; then
        echo -e "${GREEN}  PASS${NC}"
        PASSED=$((PASSED + 1))
    else
        echo -e "${RED}  FAIL${NC}"
