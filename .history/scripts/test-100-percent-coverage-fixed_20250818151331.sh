...
if [ -f "src/lib/firebase.ts" ] && [ -f "src/hooks/use-fcm.ts" ]; then
    echo -e "  ${GREEN}PASS${NC}"
    ULTRA_PASSED=$((ULTRA_PASSED + 1))
else
    echo -e "  ${RED}FAIL${NC}"
...
if [ -f "src/lib/sentry-config.ts" ]; then
    echo -e "  ${GREEN}PASS${NC}"
    ULTRA_PASSED=$((ULTRA_PASSED + 1))
else
    echo -e "  ${RED}FAIL${NC}"
...
if [ -d "src/auth" ] || [ -d "src/firestore" ] || [ -d "src/https" ] || [ -d "src/scheduled" ] || [ -d "src/storage" ] || [ -d "src/utils" ]; then
    echo "🔧 Correction automatique des Firebase Functions..."
    cd ..
    ../../scripts/fix-firebase-functions.sh
    cd functions
fi
...
if [ -f "test-output-ultra-deep/frontend/src/lib/sentry-config.ts" ]; then
    echo -e "  ${GREEN}PASS${NC}"
    ADVANCED_PASSED=$((ADVANCED_PASSED + 1))
else
    echo -e "  ${RED}FAIL${NC}"
