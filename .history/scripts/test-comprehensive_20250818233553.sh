...
TOTAL=$((TOTAL + 1))
if [ -d "test-output-minimal-mui/backend/extensions" ]; then
    echo -e "${GREEN}  PASS${NC}"
    PASSED=$((PASSED + 1))
else
    echo -e "${RED}  FAIL${NC}"
...
TOTAL=$((TOTAL + 1))
LONG_NAME="test-project-with-a-very-long-name-that-exceeds-normal-limits-and-should-still-work-properly"
...
TOTAL=$((TOTAL + 1))
SPECIAL_NAME="test-project-123_456-789"
