...
if npx ts-node "$REPO_ROOT/src/cli.ts" create --name test-complete-shadcn --description "Complete test project Shadcn" --author "Test" --package-manager npm --nextjs-version 15 --ui shadcn --state-management redux --features pwa,fcm,analytics,performance,sentry --output ./test-output-complete-shadcn --yes; then
    echo -e "${GREEN}  PASS${NC}"
    PASSED=$((PASSED + 1))
