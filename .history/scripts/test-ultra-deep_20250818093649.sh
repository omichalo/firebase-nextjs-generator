...
if node dist/cli.js create \
    ...
    run_test "Advanced features" "test -f ./test-output-ultra-deep/frontend/src/lib/firebase.ts && test -f ./test-output-ultra-deep/frontend/src/stores/redux/store.ts"
    run_test "Handlebars processing (ultra-complete)" "grep -q 'test-ultra-deep' ./test-output-ultra-deep/frontend/package.json"
    run_test "Project name replacement (ultra-complete)" "grep -q 'test-ultra-deep' ./test-output-ultra-deep/frontend/next.config.js"
...
if curl -s http://localhost:3000 > /dev/null 2>&1; then
    ...
    run_test "Home page accessible" "curl -s http://localhost:3000 | grep -q 'test-ultra-deep'"
    run_test "Login page accessible" "curl -s http://localhost:3000/auth/login | grep -q 'Connexion'"
    run_test "Dashboard page accessible" "curl -s http://localhost:3000/dashboard | grep -q 'Dashboard'"
    run_test "404 page accessible" "curl -s http://localhost:3000/nonexistent | grep -q 'Page introuvable'"
...
run_test "Redux store configuration" "test -f ./test-output-ultra-deep/frontend/src/stores/redux/store.ts"
run_test "Redux auth slice" "test -f ./test-output-ultra-deep/frontend/src/stores/redux/auth-slice.ts"
