...
if [ -d "$TEST_BASE_DIR/test-output-yarn-14" ] && \
   ...
   [ -f "$TEST_BASE_DIR/test-output-yarn-14/frontend/package.json" ] && \
   [ -f "$TEST_BASE_DIR/test-output-minimal-mui/frontend/package.json" ] && \
   grep -q "\"next\": \"^14" "$TEST_BASE_DIR/test-output-yarn-14/frontend/package.json" && \
   grep -q "\"next\": \"^15" "$TEST_BASE_DIR/test-output-minimal-mui/frontend/package.json"; then
    echo -e "${GREEN}  PASS${NC}"
    PASSED=$((PASSED + 1))
else
    echo -e "${RED}  FAIL${NC}"
...
if [ -f "$TEST_BASE_DIR/test-output-minimal-mui/frontend/package-lock.json" ] && \
   [ -f "$TEST_BASE_DIR/test-output-yarn-14/frontend/yarn.lock" ] && \
   [ -f "$TEST_BASE_DIR/test-output-minimal-mui/frontend/package.json" ] && \
   [ -f "$TEST_BASE_DIR/test-output-yarn-14/frontend/package.json" ]; then
    echo -e "${GREEN}  PASS${NC}"
    PASSED=$((PASSED + 1))
else
    echo -e "${RED}  FAIL${NC}"
