...
if node dist/cli.js create \
    --project-name "test-firebase-real" \
    --output-dir "./test-firebase-real" \
...
run_test "Auth emulator running" "curl -s http://localhost:9099 | grep -q 'Firebase Auth Emulator'"
run_test "Firestore emulator running" "curl -s http://localhost:8080 | grep -q 'Firestore Emulator'"
run_test "Functions emulator running" "curl -s http://localhost:5001 | grep -q 'Functions Emulator'"
run_test "Storage emulator running" "curl -s http://localhost:9199 | grep -q 'Storage Emulator'"
