...
TOTAL=$((TOTAL + 1))
if [ -f "$TEST_BASE_DIR/test-output-minimal-mui/frontend/src/stores/auth-store.ts" ] && grep -q "create" "$TEST_BASE_DIR/test-output-minimal-mui/frontend/src/stores/auth-store.ts" && grep -q "export.*useAuthStore" "$TEST_BASE_DIR/test-output-minimal-mui/frontend/src/stores/auth-store.ts"; then
    echo -e "${GREEN}  PASS${NC}"
    PASSED=$((PASSED + 1))
else
    echo -e "${RED}  FAIL${NC}"
