...
# Utiliser le projet de test existant ou le créer si nécessaire
echo -e "\n${YELLOW}Checking existing test project or creating new one...${NC}"

if [ ! -d "./test-firebase-real" ]; then
    echo -e "${BLUE}Creating new test project...${NC}"
    ...
    if node dist/cli.js create \
        ...
        echo -e "${GREEN}✅ Projet de test créé avec succès!${NC}"
    else
        echo -e "${RED}❌ Échec de la création du projet de test${NC}"
        exit 1
    fi
else
