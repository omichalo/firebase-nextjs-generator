...
if [ -f "src/stores/store.ts" ] && [ -f "src/stores/auth-slice.ts" ] && [ -f "src/hooks/use-auth.ts" ]; then
    echo -e "  ${GREEN}PASS${NC}"
    ULTRA_PASSED=$((ULTRA_PASSED + 1))
else
    echo -e "  ${RED}FAIL${NC}"
...
if [ -f "src/lib/sentry-config.ts" ] && [ -f "src/middleware.ts" ]; then
    echo -e "  ${GREEN}PASS${NC}"
    ULTRA_PASSED=$((ULTRA_PASSED + 1))
else
    echo -e "  ${RED}FAIL${NC}"
...
if [ -d "functions/src/auth" ] || [ -d "functions/src/firestore" ] || [ -d "functions/src/https" ] || [ -d "functions/src/scheduled" ] || [ -d "functions/src/storage" ] || [ -d "functions/src/utils" ]; then
    ...
    # Exécuter le script de correction depuis le répertoire backend
...
if [ -f "test-output-ultra-deep/frontend/src/lib/sentry-config.ts" ] && [ -f "test-output-ultra-deep/frontend/src/middleware.ts" ]; then
    echo -e "  ${GREEN}PASS${NC}"
    ADVANCED_PASSED=$((ADVANCED_PASSED + 1))
else
    echo -e "  ${RED}FAIL${NC}"
