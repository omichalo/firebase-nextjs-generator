...
print_header() {
    ...
    echo -e "\033[0;34m║                    🧪 PLAN DE TEST COMPLET                    ║\033[0m"
    echo -e "\033[0;34m║              Générateur Firebase + Next.js 2025              ║\033[0m"
    echo -e "\033[0;34m╚══════════════════════════════════════════════════════════════╝\033[0m"
    echo
}
...
print_test() {
    local test_id="$1"
    local description="$2"
    echo -e "\033[0;36m🔄 $test_id\033[0m - $description"
}

print_success() {
    local test_id="$1"
    local message="$2"
    echo -e "\033[0;32m✅ $test_id\033[0m - $message"
    ((PASSED_TESTS++))
...
print_failure() {
    ...
    local message="$2"
    local error="$3"
    echo -e "\033[0;31m❌ $test_id\033[0m - $message"
    if [[ -n "$error" ]]; then
        echo -e "\033[0;31m   Erreur: $error\033[0m"
    fi
    ((FAILED_TESTS++))
...
trap cleanup EXIT
trap 'echo -e "\n\033[0;31m❌ Test interrompu par l'utilisateur\033[0m"; cleanup; exit 1' INT TERM
...
check_prerequisites() {
    ...
    if command -v firebase &> /dev/null; then
        ...
        print_warning "Firebase CLI non installe optionnel pour les tests"
    fi
}
...
generate_report() {
    ...
    echo -e "\033[0;34m╔══════════════════════════════════════════════════════════════╗\033[0m"
    echo -e "\033[0;34m║                        📊 RAPPORT FINAL                      ║\033[0m"
    echo -e "\033[0;34m╚══════════════════════════════════════════════════════════════╝\033[0m"
    echo
    
    echo -e "\033[0;36m📈 Résultats des tests:\033[0m"
    ...
    echo -e "\033[0;36m📊 Taux de réussite: ${success_rate}%\033[0m"
    echo
    
    if [[ $success_rate -eq 100 ]]; then
        echo -e "\033[0;32m🎉 Tous les tests ont réussi !\033[0m"
        echo -e "\033[0;32m🚀 Le générateur est prêt pour la production !\033[0m"
    elif [[ $success_rate -ge 80 ]]; then
        echo -e "\033[1;33m⚠️  La plupart des tests ont réussi.\033[0m"
        echo -e "\033[1;33m🔧 Quelques ajustements mineurs peuvent être nécessaires.\033[0m"
    else
        echo -e "\033[0;31m❌ Trop de tests ont échoué.\033[0m"
        echo -e "\033[0;31m🚨 Une révision majeure est nécessaire.\033[0m"
    fi
    echo
    
    # Afficher les détails des échecs
    if [[ $FAILED_TESTS -gt 0 ]]; then
        echo -e "\033[0;31m❌ Tests échoués:\033[0m"
