...
echo -e "${BLUE}Testing: Shadcn components generation${NC}"
TOTAL=$((TOTAL + 1))
if [ -f "test-output-complete-shadcn/frontend/src/components/Button.tsx" ] && [ -f "test-output-complete-shadcn/frontend/src/components/Card.tsx" ]; then
    echo -e "${GREEN}  PASS${NC}"
    PASSED=$((PASSED + 1))
else
    echo -e "${RED}  FAIL${NC}"
...
TOTAL=$((TOTAL + 1))
if [ -f "test-output-complete-shadcn/frontend/src/stores/auth-slice.ts" ]; then
    echo -e "${GREEN}  PASS${NC}"
    PASSED=$((PASSED + 1))
else
    echo -e "${RED}  FAIL${NC}"
