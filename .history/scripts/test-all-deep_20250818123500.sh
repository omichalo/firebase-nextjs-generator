...
# Exécuter avec traces (compatible macOS)
debug_log "Starting test-ultra-deep.sh..."
debug_log "Executing test-ultra-deep.sh with real-time output..."

# Exécuter en temps réel et capturer le log
if "$SCRIPT_DIR/test-ultra-deep.sh" 2>&1 | tee test-ultra-deep.log; then
    ...
    ULTRA_PASSED=$(grep -c "  PASS" test-ultra-deep.log 2>/dev/null || echo "0")
    ULTRA_FAILED=$(grep -c "  FAIL" test-ultra-deep.log 2>/dev/null || echo "0")
    ...
    echo -e "${RED}❌ Test ultra-complet a échoué ou timeout${NC}"
    debug_log "Test failed or timed out. Checking log file..."
    
    if [ -f "test-ultra-deep.log" ]; then
        echo "Log du test ultra-complet :"
        cat test-ultra-deep.log
        debug_log "Log file exists and contains $(wc -l < test-ultra-deep.log) lines"
    else
        echo "Aucun log trouvé - le script n'a probablement pas démarré"
        debug_log "No log file found - script probably didn't start"
    fi
    
    # Tentative de récupération : vérifier si le projet a été généré
    if [ -d "./test-output-ultra-deep" ]; then
        debug_log "Project directory exists, attempting manual validation..."
        echo "Tentative de validation manuelle du projet généré..."
        
        # Validation manuelle basique
        if [ -f "./test-output-ultra-deep/frontend/package.json" ]; then
            echo "✅ Projet frontend détecté"
            ULTRA_PASSED=1
            ULTRA_FAILED=0
        else
            echo "❌ Projet frontend incomplet"
            ULTRA_PASSED=0
            ULTRA_FAILED=1
        fi
    else
        echo "❌ Aucun projet généré trouvé"
        ULTRA_PASSED=0
        ULTRA_FAILED=1
    fi
    
    show_results "Test Ultra-Complet (Récupération)" "$ULTRA_PASSED" "$ULTRA_FAILED"
fi
