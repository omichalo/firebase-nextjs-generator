...
TOTAL=$((TOTAL + 1))
if [ -f "test-output-minimal-mui/frontend/src/hooks/use-auth.ts" ] && grep -q "export.*useAuth" "test-output-minimal-mui/frontend/src/hooks/use-auth.ts" && grep -q "useState\|useEffect" "test-output-minimal-mui/frontend/src/hooks/use-auth.ts"; then
    echo -e "${GREEN}  PASS${NC}"
    PASSED=$((PASSED + 1))
else
    echo -e "${RED}  FAIL${NC}"
...
TOTAL=$((TOTAL + 1))
if [ -f "test-output-minimal-mui/frontend/src/stores/auth-store.ts" ] && grep -q "create.*store" "test-output-minimal-mui/frontend/src/stores/auth-store.ts" && grep -q "export.*useAuthStore" "test-output-minimal-mui/frontend/src/stores/auth-store.ts"; then
    echo -e "${GREEN}  PASS${NC}"
    PASSED=$((PASSED + 1))
else
    echo -e "${RED}  FAIL${NC}"
