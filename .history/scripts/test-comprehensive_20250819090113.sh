...
# Test 8.1: Génération des composants MUI
...
# Test 8.3: Génération des stores Zustand
echo -e "${BLUE}Testing: Zustand stores generation${NC}"
TOTAL=$((TOTAL + 1))
cd "$TEST_BASE_DIR/test-output-minimal-mui/frontend"

# Vérifier que les stores Zustand sont syntaxiquement valides et peuvent être compilés
if [ -f "src/stores/auth-store.ts" ] && \
   grep -q "import.*zustand" "src/stores/auth-store.ts" && \
...
# Test 8.5: Génération des hooks personnalisés
