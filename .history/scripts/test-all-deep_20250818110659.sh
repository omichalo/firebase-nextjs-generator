...
if "$SCRIPT_DIR/test-ultra-deep.sh" > test-ultra-deep.log 2>&1; then
    ...
    ULTRA_PASSED=$(grep -o "Passed: [0-9]*" test-ultra-deep.log | tail -1 | grep -o "[0-9]*")
    ULTRA_FAILED=$(grep -o "Failed: [0-9]*" test-ultra-deep.log | tail -1 | grep -o "[0-9]*")
...
if ./scripts/test-firebase-real.sh > test-firebase-real.log 2>&1; then
...
if ./scripts/test-advanced-features.sh > test-advanced-features.log 2>&1; then
