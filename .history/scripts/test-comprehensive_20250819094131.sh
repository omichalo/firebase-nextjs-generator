...
if [ -d "functions" ] && \
   ...
   grep -q "exports" "functions/src/index.ts"; then
    echo -e "${GREEN}  PASS${NC}"
    PASSED=$((PASSED + 1))
else
    echo -e "${RED}  FAIL${NC}"
...
if [ -d "functions/src/auth" ] && \
   ...
   grep -q "auth.user.onCreate" "functions/src/auth/user-created.ts" && \
   grep -q "auth.user.onUpdate" "functions/src/auth/user-updated.ts" && \
   grep -q "auth.user.onDelete" "functions/src/auth/user-deleted.ts"; then
    echo -e "${GREEN}  PASS${NC}"
    PASSED=$((PASSED + 1))
else
    echo -e "${RED}  FAIL${NC}"
...
if [ $FAILED -eq 0 ]; then
    ...
    echo -e "  ✅ Tests de sécurité"
        ...
        echo -e "  ✅ Validation de la configuration Firebase pour déploiement"
        echo -e "  ✅ Validation de la configuration Next.js pour déploiement"
        echo -e "  ✅ Test de build de production pour déploiement"
        echo -e "  ✅ Validation des assets de production"
        echo -e "  ✅ Validation de la configuration d'environnement"
        echo -e "  ✅ Tests d'intégration Firebase réels"
        echo -e "  ✅ Validation de la structure des Cloud Functions"
        echo -e "  ✅ Validation de la configuration Firestore"
        echo -e "  ✅ Validation de la configuration Storage"
        echo -e "  ✅ Validation des extensions Firebase"
        echo -e "  ✅ Validation de la configuration d'authentification"
    exit 0
else
