...
export class ConfigValidator {
  ...
  static validateFirebaseConfig(config: FirebaseConfig): ValidationResult {
    ...
    // Validation des projets et régions au niveau des environnements
    for (const env of config.environments) {
      // Validation du project ID de l'environnement
      if (!env.projectId || !/^[a-z0-9-]+$/.test(env.projectId)) {
        errors.push(
          `Project ID Firebase invalide pour l'environnement ${env.name}: ${env.projectId}`
        );
      }

      // Validation de la région de l'environnement
      if (env.region && env.region !== "existing") {
        const validRegions = [
          "us-central1",
          "us-east1",
          "us-west1",
        ...
        if (!validRegions.includes(env.region)) {
          warnings.push(
            `Région Firebase non standard pour ${env.name}: ${env.region}`
          );
        }
      }
    }
