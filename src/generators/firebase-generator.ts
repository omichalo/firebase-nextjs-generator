...
export class FirebaseGenerator {
  ...
  private async generateFirebaseConfig(outputDir: string): Promise<void> {
    // firebase.json (sans hosting statique)
    ...
    // firebase-app-hosting.json (pour App Hosting avec API routes)
    await this.templateEngine.processTemplate(
      path.join(
        this.options.templateDir,
        'firebase/firebase-app-hosting.json.hbs'
      ),
      path.join(outputDir, 'firebase-app-hosting.json')
    );
  ...
  private async generateDeploymentScripts(outputDir: string): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        console.log('Template rollback.sh non trouvé, ignoré');
      }
